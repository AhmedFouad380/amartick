...
//        where('sender_id', Auth::user()->id)->
    ...
    {
        $inbox_count = Inbox::where('receiver_type', 'user')->where('receiver_id', Auth::user()->id)->where('is_read', 0)->count();
        $order_count = Order::where('user_id', Auth::user()->id)->where('type', 'Accepted')->count();
        $data = [];
